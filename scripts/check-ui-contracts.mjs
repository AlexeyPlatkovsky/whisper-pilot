...
function readOrderedCss(relative) {
  ...
  return [
    source,
    ...imports.map((match) =>
      readOrderedCss(path.normalize(path.join(path.dirname(relative), match[1]))),
    ),
  ].join("\n");
}
...
for (const file of walk("src").filter((entry) => entry.endsWith(".tsx"))) {
  ...
  if (/<[A-Za-z][^>]*\brole\s*=\s*["']alertdialog["']/s.test(source)) {
...
const styles = readOrderedCss("src/styles.css");
if (!/\.modal-button--danger\s*\{[^}]*var\(--error\)/s.test(styles)) {
  errors.push("src/styles.css: destructive modal action must use var(--error)");
}
const modalSafeSettingsSelectors = [
  /\.settings-content button:not\(\.modal-button\):not\(\.model-icon-btn\),\s*\.settings-header button\s*\{/s,
  /\.settings-content\s+button:not\(\.modal-button\):not\(\.model-icon-btn\):hover:not\(:disabled\),\s*\.settings-header button:hover:not\(:disabled\)\s*\{/s,
  /\.settings-content button:not\(\.modal-button\):not\(\.model-icon-btn\):disabled\s*\{/s,
];
if (modalSafeSettingsSelectors.some((selector) => !selector.test(styles))) {
  errors.push(
    "src/styles.css: settings base, hover, and disabled rules must exclude shared modal buttons",
  );
}
