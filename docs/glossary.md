...
| **Live translation** | Streaming's translation of the running transcript into a chosen target language, generated by the same local llama.cpp/Qwen model used for summarization while the session is still capturing (ADR-015, ADR-016). A separate capability from transcription itself — it never changes what Whisper detects, only adds a translated reading alongside it. |
| **Target language** | The language a Streaming session's transcript is translated into when Live Translation is on: English or Russian only. Distinct from the **transcription language**, which stays auto-detected per window and is never chosen (ADR-012, ADR-014); the target is a separate, user-picked setting that exists only for translation. |
...
| **Paragraph (display/alignment unit)** | The unit the split view groups and aligns rows on — one entry from Streaming's `groupWindowsIntoParagraphs`. A paragraph and its translated cell always render at the same vertical position; the cell is built by joining its windows' own translations, filling in as each one finishes (ADR-016). Distinct from the **window (translation unit)** above, which is what actually gets sent to the model and persisted. |
