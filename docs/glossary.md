...
| **Live translation** | Streaming's per-paragraph translation of the running transcript into a chosen target language, generated by the same local llama.cpp/Qwen model used for summarization while the session is still capturing (ADR-015). A separate capability from transcription itself — it never changes what Whisper detects, only adds a translated reading alongside it. |
...
| **Paragraph (translation unit)** | The unit Live Translation translates and aligns on — one entry from Streaming's `groupWindowsIntoParagraphs`, not a window or a sentence. A paragraph and its translation always render at the same vertical position in the split view, and translations persist keyed by session, paragraph, and target language (ADR-015). |
