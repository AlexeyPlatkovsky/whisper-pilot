...
| `architecture.md` | Architecture map and routing to functional technical boundaries | You need to locate the owning architecture area |
| `architecture/` | Functional architecture: Transcription, Meeting, Recorder, local AI, platform, desktop/privacy/build | You need technical constraints for a feature area |
...
| `roadmap.md` | Version milestones v1.30–v1.50 and capability sequencing | You need the release plan or priorities |
| `glossary.md` | Domain vocabulary (Transcription, Meeting, Recorder, library, diarization, …) | You hit an unfamiliar term |
...
| ADR-002 | Offline full-file (batch) Transcription, not live capture | partially superseded by ADR-014 (app-wide scope boundary only; the full-file batch decision and its accuracy rationale for Transcription stand) |
...
| ADR-008 | Persisted Transcription library (SQLite), reference-only audio, auto-save | accepted |
| ADR-009 | Structured MFU (full set), editable | accepted |
| ADR-010 | Two-pane Transcription workspace shell with manual Transcribe/MFU triggers | accepted |
...
| ADR-014 | Meeting live capture coexists with Transcription's batch-accuracy pipeline | accepted |
