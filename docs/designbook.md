...
- These tokens are derived directly from the approved design at
  ...
  code counterpart.
- `tokens.css` is imported before the `styles.css` entry point (see `src/main.tsx`), so the
...
- Light/dark **theming** is layered on top of the raw tokens as *semantic*
  variables (`--bg`, `--panel`, `--text`, `--accent`, …) in `styles/base.css`. The
  tokens describe the design's base (light) palette; the dark palette is derived
  in `styles/base.css`. An explicit Light/Dark choice (Settings → Appearance) wins
...
- status colours: success green, error red, in-progress teal-blue, plus two
  ...
  `--wp-attention` (dark amber, for a Transcription with no source file, which the
...
- one Transcription status is shown in two places at once — the sidebar row's dot and
...
density trade-off, chosen so the row shows only what distinguishes one Transcription
...
Transcription — `title` tooltips are not reachable by keyboard or touch. The
open Transcription's status is always spelled out in the header. Revisit by revealing the
